...
class Task(object):
    def __init__(self):
        ...
        self.target_all_state = np.zeros(9)
    ...
    def cal_target_expanded_state(self, target_state):
        ...
        self.target_expanded_state = psix(self.target_all_state)
    ...
    def get_linearization_from_trajectory(self, trajectory, actions):
        return [self.ldx(state, action) for state, action in zip(trajectory, actions)], [self.ldu(state, action) for
                                                                                         state, action in
