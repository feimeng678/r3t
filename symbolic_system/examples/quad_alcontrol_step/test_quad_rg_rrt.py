...
from polytope_symbolic_system.examples.quadcopter import Quadcopter
from r3t.symbolic_system.symbolic_system_rg_rrt import SymbolicSystem_RGRRT
...
from r3t.symbolic_system.examples.quad_alcontrol_step import NTC_alcontrol
matplotlib.rcParams['font.family'] = "Times New Roman"

global best_distance


def test_quadcopter_planning():
    global best_distance
    best_distance = np.inf
    R = euler2mat(np.random.uniform(-1.,1., size=(3,)))
    p = np.array([0., 0., 0.])
    g = RpToTrans(R, p).ravel()
    twist = np.random.uniform(-1., 1., size=(6,)) #* 2.0
    initial_state = np.concatenate((g, twist))
    # initial_state = np.zeros(22)
    quadcopter_system = Quadcopter(initial_state=initial_state, input_limits=np.tile(np.asarray([[-6],[6]]),(1,4)))
    goal_state = np.asarray([0., 0., -9.81, 0, 0, 0, 0, 0, 0])
    ...
    #fixme reasonable
    def uniform_sampler():
        rnd = np.random.rand(9)
        rnd[0] = rnd[0] -9.81
        rnd[1] = rnd[1] -9.81
        rnd[2] = rnd[2] -9.81
        # rnd[3] = (rnd[3] - 0.5) * 2 * 12
        # rnd[4] = (rnd[4] - 0.5) * 2 * 12
        # rnd[5] = (rnd[5] - 0.5) * 2 * 12
        # rnd[6] = (rnd[6] - 0.5) * 2 * 12
        # rnd[7] = (rnd[7] - 0.5) * 2 * 12
        # rnd[8] = (rnd[8] - 0.5) * 2 * 12
    ...
    def reached_goal_function(state, goal_state):
        global best_distance
        d1 = np.linalg.norm(NTC_alcontrol.get_measurement(state)-goal_state)
        # d2 = np.linalg.norm(state-goal_state_2)
        best_distance = min(best_distance, d1)
        if d1<5e-2: # or d2<5e-2
            print('Goal error %f' %np.linalg.norm(state-goal_state)) #, np.linalg.norm(state-goal_state_2)
            return True
        return False
    ...
    def arrow_plot(soa):
        ...
        plt.title('RG-RRT after %.2f seconds (explored %d nodes) for Quad Stabilization' % (duration, rrt.node_tally))
        plt.savefig('./RG_figures/Quad/RG_RRT_Pendulum_'+experiment_name+'/%.2f_seconds_tree.png' % duration, dpi=500)
        plt.clf()
        plt.close()

    rrt = SymbolicSystem_RGRRT(quadcopter_system, uniform_sampler, step_size, reached_goal_function=reached_goal_function, linearize=True)
    # if linearize:
    ...
    duration = 0
    os.makedirs('./RG_figures/Quad/RG_RRT_Quadcopter_'+experiment_name)
    while(1):
        start_time = time.time()
        if rrt.build_tree_to_goal_state(goal_state,stop_on_first_reach=True, allocated_time= 100, rewire=False,
                                        explore_deterministic_next_state=False) is not None:
            found_goal = True
        end_time = time.time()

        if found_goal:
            p = rrt.goal_node.parent.state
            goal_override = np.asarray([0., 0., -9.81])
        ...
        quad_posi_list = np.asarray(rrt.posi_list)
        quad_orie_list = np.asarray(rrt.orie_list)
        posi_orie = np.concatenate((quad_posi_list, quad_orie_list), axis=1)
        ax.scatter(initial_state[0], initial_state[1], initial_state[2], facecolor='red', s=5)
        ax.scatter(goal_state[0], goal_state[1], goal_state[2], facecolor='green', s=5)
        # ax.scatter(goal_state[0]-2*np.pi, goal_state[1], facecolor='green', s=5)
        ax.grid(True, which='both')
        y_formatter = matplotlib.ticker.ScalarFormatter(useOffset=False)
        ax.yaxis.set_major_formatter(y_formatter)
        ax.set_yticks(np.arange(-10, 10, 2))
        arrow_plot(posi_orie)
