...
def quad_alcontrol_step(starting_state=None, target_state=None, states_list=None):

    quad = Quadcopter() ### instantiate a quadcopter
    ...
    # coordinate_list = get_position(state)
    for t in range(simulation_time):

        #### measure state and transform through koopman observables
        m_state = get_measurement(state) #22->9

        t_state = koopman_operator.transform_state(m_state) #9->18
        # err[t] = np.linalg.norm(m_state[:3] - target_orientation) + np.linalg.norm(m_state[3:])
        err[t, :] = np.array([m_state[:3] - target_state])
        ...
        lqr_policy.set_target_state(task.cal_target_expanded_state(target_state))
        lqr_policy.sat_val = quad.sat_val ### set the saturation value
        ...
        # coordinate_list.append(get_position(state))
